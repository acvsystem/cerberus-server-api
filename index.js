...
io.on('connection', async (socket) => {
  ...
  app.post("/frontRetail/search/huellero", async (req, res) => {
    ...
    (dataServGeneral || []).filter(async (huellero, i) => {
      ...
      let day = fechaFormateada.split('/');
      let parseDate = `${day[0]}-${parseInt(day[1]) * 1}-${parseInt(day[2]) * 1}`;
      console.log(`*************SELECT * FROM TB_HEAD_PAPELETA WHERE ESTADO_PAPELETA != 'anulado' AND ID_PAP_TIPO_PAPELETA = 7 AND NRO_DOCUMENTO_EMPLEADO = '${(huellero || {}).nroDocumento}' AND FECHA_DESDE = '${(huellero || {}).dia}';`);
