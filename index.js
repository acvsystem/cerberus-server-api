...
const task_12 = cron.schedule('00 15 * * 0', () => {
  console.log('00 15 * * 0','onEmitJobMallAventura(1)');
  console.log('00 15 * * 0')
  onEmitJobMallAventura(1);
});

const task_13 = cron.schedule('01 15 * * 0', () => {
  console.log('01 15 * * 0','onEmitJobMallAventura(2)');
  onEmitJobMallAventura(2);
});

const task_14 = cron.schedule('02 15 * * 0', () => {
  console.log('02 15 * * 0','onEmitJobMallAventura(3)');
  onEmitJobMallAventura(3);
});
...
function emitClearClient(code) {
  ...
  pool.query(`SELECT * FROM TB_CLIENTES_CLEAR_FORNT;`).then(([data]) => {
    ...
    io.emit("limpiarCliente", { clientes: listCliente, codeStore: code || "backend", isCallGoblar: false }, socketID || "backend");
  });
}
...
function onVerificarCalendario() {
  ...
  pool.query(`SELECT CODIGO_TIENDA FROM TB_HORARIO_PROPERTY WHERE TRIM(SUBSTRING_INDEX(RANGO_DIAS, ' ', 1)) = '${parseInt(day[0])}-${parseInt(day[1])}-${parseInt(day[2])}' GROUP BY CODIGO_TIENDA;`).then(([calendarios]) => {
...
io.on('connection', async (socket) => {
  ...
  socket.on('reportSalesDepartament', (configuracion) => {
    socket.broadcast.emit('reportGetSaleDepartament', configuracion, (socket || {}).id);
  });
  ...
  app.post("/frontRetail/search/huellero", async (req, res) => {
    ...
    (dataServGeneral || []).filter(async (huellero, i) => {
      ...
      let day = fechaFormateada.split('/');
      let parseDate = `${parseInt(day[0]) * 1}-${parseInt(day[1]) * 1}-${parseInt(day[2]) * 1}`;
  ...
  function onSearchDescanso(data, index, day, number_indentity, length, socketID) {
    ...
    pool.query(`SELECT * FROM TB_DIAS_LIBRE 
      ...
      pool.query(`SELECT * FROM TB_HEAD_PAPELETA WHERE ESTADO_PAPELETA != 'anulado' AND ID_PAP_TIPO_PAPELETA = 7 AND NRO_DOCUMENTO_EMPLEADO = '${number_indentity}' AND FECHA_DESDE = '${day}';`).then(([papeleta]) => {
        ((data || [])[indexRow] || {})['papeleta'] = papeleta || [];
