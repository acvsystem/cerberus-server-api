...
io.on('connection', async (socket) => {
  ...
  app.post("/calendario/generar", async (req, res) => {
    ...
    let [cargosList] = await pool.query(`SELECT * FROM TB_HORARIO_PROPERTY WHERE RANGO_DIAS = '${rs.rango}' AND CODIGO_TIENDA = '${rs.codigo_tienda}';`);
    ...
    dataHorario.filter(async (dth) => {
      ...
      dth['rg_hora'].filter(async (rangoh) => {
        if ((rangoHora || []).length) {
          ...
          await pool.query(`UPDATE TB_RANGO_HORA SET RANGO_HORA='${rangoh.rg}' WHERE ID_RANGO_HORA = ${(rangoHoraSelected[0] || []).ID_RANGO_HORA};`);
        } else {
