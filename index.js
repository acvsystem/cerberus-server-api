...
io.on('connection', async (socket) => {
  ...
  app.post("/comparacion/bdTienda", async (req, res) => {
    io.timeout(2000).emit("comparacionServer", 'DATA', async (err, response) => {
      ...
      console.log("comparacionServer",JSON.parse(dataPlugin[0].DATA));
      let dataResponse = await facturacionController.verificacionCoeData(JSON.parse(dataPlugin[0].DATA));
