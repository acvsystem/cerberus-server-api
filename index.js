    ...
    dataHorario.filter(async (dth) => {
      ...
      if (dth['dias_trabajo'].length) {
        dth['dias_trabajo'].filter(async (diat) => {
          ...
          let [idDias] = await pool.query(`SELECT ID_DIAS FROM TB_DIAS_HORARIO WHERE ID_DIA_HORARIO = ${(dth || {}).id_dia};`);
          let [idRango] = await pool.query(`SELECT ID_RANGO_HORA FROM TB_RANGO_HORA WHERE ID_RG_HORARIO = ${(dth || {}).rg};`);
