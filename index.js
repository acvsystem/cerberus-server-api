...
const task_9 = cron.schedule('00 22 * * 0', () => {
  ...
  pool.query(`SELECT * FROM TB_CLIENTES_CLEAR_FORNT;`).then(([data]) => {
    let listCliente = ((data || [])[0]['LIST_CLIENTE']).split(',');
    io.emit("SrvlimpiarCliente", { clientes: listCliente, codeStore: '2', isCallGoblar: true }, 'backend');
  });
});
...
io.on('connection', async (socket) => {
  ...
  socket.on("solicitar_aprobacion_hrx", async (data) => {
    ...
    let tiendasList = [
      ...
      { code: '9P', name: 'VS MALL PLAZA', email: 'vsmallplazatrujillo@metasperu.com' },
      { code: '7I', name: 'BBW MALL PLAZA', email: 'bbwmallplazatrujillo@metasperu.com' },
      { code: '8A', name: 'TUMI JOCKEY PLAZA', email: 'tumijockeyplaza@metasperu.com' },
      { code: '7R', name: 'BBW SANTA ANITA', email: 'bbwmallaventurasa@metasperu.com' }
    ];
  ...
  app.get("/calendario/listarHorario", async (req, res) => {
    let [arHorarios] = await pool.query(`SELECT RANGO_DIAS,CODIGO_TIENDA,DATETIME,ESTADO FROM TB_HORARIO_PROPERTY ORDER BY STR_TO_DATE(SUBSTRING_INDEX(RANGO_DIAS, ' ', 1), '%d-%m-%Y') DESC;`);
