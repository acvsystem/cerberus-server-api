...
io.on('connection', async (socket) => {
  ...
  app.get("/calendario/listarHorario", async (req, res) => {
    let [arHorarios] = await pool.query(`SELECT SUBSTRING_INDEX(RANGO_DIAS,' ',1) FROM TB_HORARIO_PROPERTY ORDER BY  DATEDIFF(DATE(SUBSTRING_INDEX(RANGO_DIAS,' ',1)), CURDATE()) asc;`);
